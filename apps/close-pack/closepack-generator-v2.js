...
Document ID: ${certId}-COMP                    Date Prepared: ${journalDate}

───────────────────────────────────────────────────────────────────────────────
1. EU AI ACT READINESS (Articles 9, 12, 13)
───────────────────────────────────────────────────────────────────────────────

  Every AI call through Finault generates a sealed AIEI receipt with five
  fields: WHO, WHAT, WORTH, RULES, PROOF. This maps to:

  Article 9 (Risk Management):     COVERED — WORTH field tracks cost + revenue
  Article 12 (Record-keeping):     COVERED — Sealed Close Pack with SHA-256 chain
  Article 13 (Transparency):       COVERED — AIEI envelope provides full audit trail

  Compliance Status: READY (enforcement begins August 2026)

───────────────────────────────────────────────────────────────────────────────
2. SOX 404 CONTROLS
───────────────────────────────────────────────────────────────────────────────

  Control Environment:             Automated data capture, no manual entry
  Risk Assessment:                 Model pricing accuracy monitored in real-time
  Control Activities:              SHA-256 hashing, balanced journal entries
  Information & Communication:     Intelligence Reports, Slack alerts
  Monitoring:                      Anomaly detection (Z-score, IQR, EWMA, CUSUM)

  See: 04-Controls-Narrative for detailed SOX 404 mapping

───────────────────────────────────────────────────────────────────────────────
3. COLORADO SB205 COMPLIANCE
───────────────────────────────────────────────────────────────────────────────

  Per-agent Economic Cards:        Agent-level cost tracking with X-Finault-Agent-Id
  Accountability:                  Per-agent budget enforcement (HARD_CAP/SOFT_CAP/ALERT)
  Transparency:                    Agent spend visible in /v1/agents endpoint

───────────────────────────────────────────────────────────────────────────────
4. DATA RETENTION
───────────────────────────────────────────────────────────────────────────────

  Retention Period:                7 years per IRS Publication 583
  Primary Archive:                 Cloudflare R2 (finault-closepacks bucket)
  Secondary Archive:               Supabase (close_packs table)
  Integrity Verification:          SHA-256 chain hash across all periods

───────────────────────────────────────────────────────────────────────────────
5. CHAIN INTEGRITY
───────────────────────────────────────────────────────────────────────────────

  Current Period Hash:             ${dataHash}
  Chain Validation:                Each Close Pack includes prior pack's hash
  Tamper Detection:                Any modification invalidates all subsequent hashes

  chain_hash = SHA-256(prior_close_id : close_id : prior_chain_hash : data_hash)

═══════════════════════════════════════════════════════════════════════════════
                              CONFIDENTIAL
    This document contains proprietary compliance information of ${companyName}.
              Unauthorized distribution is prohibited.
                         Generated by Finault
═══════════════════════════════════════════════════════════════════════════════
`;
  ...
  const files = [
    ...
    ['06-COMPLIANCE-REPORT.txt', complianceReport],
    ['07-QUICKBOOKS-IMPORT.iif', qbIIF.replace(/\\t/g, '\t')],
    ['08-XERO-IMPORT.csv', xeroCSV],
    ['09-SAGE-IMPORT.csv', sageCSV],
    ['10-LINE-ITEM-DETAIL.csv', detailCSV]
  ];
