-- Migration: Add Error Tracking System (SIMPLIFIED)
-- Date: 2026-02-07
-- Purpose: Replace silent .catch(() => {}) failures with proper error tracking
-- Note: Minimal RLS policies - works with any schema
...
DROP POLICY IF EXISTS "Authenticated users can view errors" ON error_logs;

-- Service role has full access (this is what the gateway uses)
CREATE POLICY "Service role full access" ON error_logs
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- Authenticated users can view all errors (simplified - can be tightened later)
...
BEGIN
  ...
  SELECT
    ...
    ROUND(COUNT(*)::NUMERIC / NULLIF(hours_ago, 0), 2) as error_rate_per_hour
  FROM error_logs
  WHERE
...
DO $$
BEGIN
  RAISE NOTICE '✅ Error tracking migration completed successfully!';
  RAISE NOTICE 'Created: error_logs table, 6 indexes, 2 RLS policies, 2 helper functions';
END $$;
