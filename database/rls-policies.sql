...
-- FIX 4 (HIGH): Explicit NULL handling - returns NULL if user not found in users table
-- This is intentional: unregistered auth users get no data access (fail-secure deny-by-default)
CREATE OR REPLACE FUNCTION get_current_user_org()
RETURNS UUID AS $$
DECLARE
    v_org_id UUID;
BEGIN
    SELECT organization_id INTO v_org_id FROM users WHERE auth_id = auth.uid();
    -- Returns NULL if user not found; RLS policies will deny access (NULL != any value)
    -- This is intentional design: unregistered auth users get no data access
    RETURN v_org_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp;

-- Add documentation for get_current_user_org helper function
COMMENT ON FUNCTION get_current_user_org() IS
'Returns the organization_id of the current authenticated user. Returns NULL if user not found,
which is intentional for fail-secure RLS policy enforcement (deny-by-default pattern).';
...
-- Add documentation for get_current_user_role helper function
COMMENT ON FUNCTION get_current_user_role() IS
'Returns the user_role of the current authenticated user (admin, finance_lead, cost_optimizer, auditor, viewer).
...
-- Add documentation for is_org_admin helper function
COMMENT ON FUNCTION is_org_admin() IS
'Returns true if the current authenticated user has admin role in their organization.
Used for access control decisions in RLS policies and security-definer functions.';
...
-- Add documentation for is_auditor helper function
COMMENT ON FUNCTION is_auditor() IS
'Returns true if the current authenticated user has auditor role.
Used for audit trail and compliance log access control in RLS policies.';
...
COMMENT ON POLICY "Users can view own organization" ON organizations IS
'Multi-tenant isolation: Users can only access their own organization record (MEDIUM: RLS tightening)';
...
COMMENT ON POLICY "Only admins can update organization" ON organizations IS
'Organization settings changes restricted to administrators only';
...
COMMENT ON POLICY "Only service role can create organizations" ON organizations IS
'Organization creation only via service role during signup process, not by users';
...
COMMENT ON POLICY "Users can view own organization members" ON users IS
'Multi-tenant isolation: Users see only other members in their organization';
...
'Users can modify only their own profile information';
...
COMMENT ON POLICY "Admins can manage organization users" ON users IS
'Administrators can modify users within their organization';
...
COMMENT ON POLICY "Admins can view user changes" ON users IS
'Admins and auditors can review user change history for compliance';
...
-- Audit trail INSERT policy
-- FIX 10 (MEDIUM→HIGH): Tighten policy to prevent unauthorized organization_id insertion
-- Note: Supabase service_role bypasses RLS entirely, so no explicit service_role check needed.
-- For authenticated users and local dev (finault_app), enforce org match.
CREATE POLICY "Users can create audit entries for own org"
...
GRANT SELECT ON invoice_line_items TO anon;
GRANT SELECT ON allocations TO anon;
