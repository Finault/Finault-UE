...
class UniversalParser {
  ...
  validate(invoiceData) {
    ...
    if (!Array.isArray(invoiceData.lineItems)) {
      ...
      if (Math.abs(invoiceData.totalAmount - calculatedTotal) > tolerance) {
        report.anomalies.push({
          severity: 'MEDIUM',
          code: 'TOTAL_MISMATCH',
          ...
          difference: invoiceData.totalAmount - calculatedTotal
        });
      }
    }
  ...
  toACPS(invoiceData) {
    const acpsInvoice = {
      ...
      lineItems: (invoiceData.lineItems || []).map((item, index) => ({
        acpsLineId: `${invoiceData.provider}-${index}`,
        originalId: item.id,
        timestamp: item.timestamp,
        provider: invoiceData.provider,
        model: item.model,
        modelType: item.type || 'unknown',
        usage: {
          metric: item.usageMetric,
          unit: item.usageUnit,
          normalized: this._normalizeUsageMetric(item)
        },
        cost: {
          amount: item.amount,
      ...
      adjustments: (invoiceData.adjustments || []).map((adj, index) => ({
        acpsAdjustmentId: `adj-${invoiceData.provider}-${index}`,
        type: adj.type || 'adjustment',
        amount: adj.amount,
        currency: invoiceData.currency,
        description: adj.description || '',
        date: adj.date || invoiceData.date
      })),
      ...
      dataQuality: {
        ...
        anomalies: this.anomalies.slice(0, 10), // First 10 anomalies
   ...
   * @param {string} filename - Filename for format hints
   * @returns {string} Detected format ('json', 'csv', 'pdf', 'text')
   */
...
// Export for use in Node.js, browser, or ES6 modules
